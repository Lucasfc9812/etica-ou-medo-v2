...
function showScene(index) {
  ...
  // Cinematic additions: REC indicator & Audio
  updateRecIndicator(SCENES[index]);
  playAmbient(SCENES[index]);
}
...
function typewriterHome() {
  ...
  document.getElementById('btn-entrar').addEventListener('click', (e) => {
    e.preventDefault();
    initAudio(); // User interacted, we can unlock Web Audio
    currentScene = 1;
...
function onVerClick() {
  ...
  setTimeout(() => {
    ...
    playGlitchSound(); // Visual glitch matches audio glitch
...
// ─────────────────────────────────────────────────────────────
// CINEMATIC UX MODULE (REC, Silence, Interaction)
// ─────────────────────────────────────────────────────────────
function updateRecIndicator(sceneId) {
  const rec = document.getElementById('rec-indicator');
  if (['caso1', 'caso2', 'caso3'].includes(sceneId)) {
...
// Play a synthetic soundscape based on the scene
function playAmbient(sceneId) {
  ...
  if (sceneId === 'home') {
  ...
  else if (sceneId === 'caso1') {
    // "Amanda" - growing notification dings
    let delay = 3500;
    let playing = true;
    ...
    const playDing = () => {
      ...
      delay = Math.max(300, delay * 0.85); // gets faster & faster

      // Stop completely at the end when she posts the video
      if (document.getElementById('narrative-amanda').lastElementChild.classList.contains('shown')) {
        playing = false;
        return;
      }
      notifInterval = setTimeout(playDing, delay);
    };
    notifInterval = setTimeout(playDing, delay);
  }
  else if (sceneId === 'caso2') {
    // "Karol" - Crowd noise turning into distorted digital noise
    ...
    noise.connect(dist).connect(filter).connect(master);
    noise.start();

    // Animate the distortion over 15 seconds
    let amt = 0;
    const distInterval = setInterval(() => {
      amt += 20;
      if (amt > 800) clearInterval(distInterval);
      dist.curve = makeDistCurve(amt);
    }, 500);

    // Proxy stop
    currentDrone = { stop: () => { noise.stop(); clearInterval(distInterval); } };
  }
  else if (sceneId === 'caso3') {
    // Silence heavy
    const osc = ctx.createOscillator();
    osc.frequency.value = 30; // barely audible rumble
    osc.connect(master);
    master.gain.value = 0.05;
    osc.start();
    currentDrone = osc;
  }
  else if (sceneId === 'fechamento') {
    // Heartbeat
    playHeartbeat(master);
  }
}

function playHeartbeat(masterGain) {
  if (!ctx) return;
  ...
  hosc.type = 'triangle';
  hosc.frequency.setValueAtTime(40, t);
  hosc.frequency.exponentialRampToValueAtTime(10, t + 0.8);

  hgain.gain.setValueAtTime(0, t);
  hgain.gain.linearRampToValueAtTime(0.4, t + 0.1);
  hgain.gain.exponentialRampToValueAtTime(0.001, t + 0.3);

  // Second beat
  hgain.gain.setValueAtTime(0, t + 0.35);
  hgain.gain.linearRampToValueAtTime(0.4, t + 0.45);
  hgain.gain.exponentialRampToValueAtTime(0.001, t + 0.8);

  hosc.connect(hgain).connect(masterGain);
  hosc.start(t);
  hosc.stop(t + 1);

  const hbTo = setTimeout(() => { playHeartbeat(masterGain); }, 1500);
  currentDrone = { stop: () => clearTimeout(hbTo) };
}
